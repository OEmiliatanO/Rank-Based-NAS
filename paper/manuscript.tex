...
% what is unknown?
    ...
    searching algorithm, we can hardly find an optimal architecture. 
...
% method
    ...
    is outlined in \xalg{alg:SA}. In line $1\sim2$, the historical best architectures 
        ...
        \begin{algorithmic}[1]
            ...
            \While{$T>\tau$}
                \For{$\text{each }i \in [1\ldots I]$}
                    ...
                    \If{$\Delta E<0$}
                        \State $s\leftarrow s'$
                    \ElsIf{$x\leq e^{-\alpha\frac{\Delta E}{T}}$}
                            \State $s\leftarrow s'$
                    \EndIf
                \EndFor
