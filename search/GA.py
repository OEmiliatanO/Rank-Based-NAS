...
class GA():
    def __init__(self, MAXN_CONNECTION, MAXN_OPERATION, searchspace, train_loader, device, stds, means, acc_type, args):
        ...
        self.logsynflow  = np.load(os.path.expanduser(f"{base_loc}/synflow_nasbench201_{args.dataset}_none_0.05_1_{args.valid}_128_1_1.npy"))
        """
        ####
    ...
    def evaluate(self):
        for i in range(len(self.population)):
            ...
            if self.population[i].gene not in self.DICT:
                self.population[i].fitness = self.DICT[self.population[i].gene] = standardize(ninaswot_score(network, self.train_loader, self.device, self.stds, self.means, self.args), self.means["ninaswot"], self.stds["ninaswot"])
            else:
